...
public class ConflictManager {
    ...
    public Conflict startConflict(ConflictType type, GameSessionContext context, DomainEventPublisher eventPublisher) {
        List<Conflict> activeConflicts = context.activeConflicts();
        if (hasActiveConflictOfType(type, activeConflicts)) {
            throw new InvalidGameStateException("Conflict of type '" + type.code() + "' is already active");
        }
    ...
    public void avoidConflict(String conflictId, GameSessionContext context, DomainEventPublisher eventPublisher) {
        Conflict conflict = findConflictById(conflictId, context.activeConflicts());
        if (conflict.stage() != ConflictStage.BREWING) {
            throw new InvalidGameStateException("Cannot avoid conflict '" + conflictId + "': not in BREWING stage");
        }
        conflict.avoidAtBrewingStage();
        eventPublisher.publish(new ConflictResolvedEvent(context.sessionId(), conflictId, "AVOIDED"));
    }

    public TacticEffects applyTactic(ConflictTactic tactic, GameSessionContext context, DomainEventPublisher eventPublisher) {
        Conflict conflict = findActiveConflict(context.activeConflicts());
        TacticEffects effects = conflict.applyTactic(tactic, context.player(), context.relationships());
        if (effects.statChanges() != null) context.player().applyStatChanges(effects.statChanges());
        if (effects.relationshipChanges() != null) {
            context.relationships().applyChanges(effects.relationshipChanges().npcCode(), effects.relationshipChanges());
        }
        eventPublisher.publish(new ConflictTacticAppliedEvent(context.sessionId(), conflict.id(), tactic.code()));
        if (conflict.isResolved()) handleConflictResolution(conflict, context, eventPublisher);
        return effects;
    }

    private void handleConflictResolution(Conflict conflict, GameSessionContext context, DomainEventPublisher eventPublisher) {
        ConflictResolution res = conflict.resolution();
        eventPublisher.publish(new ConflictResolvedEvent(context.sessionId(), conflict.id(), res.outcome().name()));
        if (res.relationshipBreak() && conflict.type().opponent().isPresent()) {
            ...
            eventPublisher.publish(new RelationshipBrokenEvent(context.sessionId(), npc.name()));
        }
    }

    private boolean hasActiveConflictOfType(ConflictType type, List<Conflict> activeConflicts) {
        return activeConflicts.stream().anyMatch(c -> c.type().code().equals(type.code()) && !c.isResolved());
    }

    private Conflict findConflictById(String conflictId, List<Conflict> activeConflicts) {
        return activeConflicts.stream().filter(c -> c.id().equals(conflictId)).findFirst()
                .orElseThrow(() -> new InvalidGameStateException("Conflict '" + conflictId + "' not found"));
    }

    private Conflict findActiveConflict(List<Conflict> activeConflicts) {
        return activeConflicts.stream().filter(c -> !c.isResolved()).findFirst()
