 ...
 * <p>Public API:
 * <ul>
 *   <li>{@link #parseOne(InputStream, String)} — parses exactly 1 quest</li>
 *   <li>{@link #parseAll(InputStream, String)} — parses 1..N quests</li>
 *   <li>{@link #parseAll(File)} — convenience overload for unit tests</li>
 * </ul>
 *
...
public class QuestSpecParser {
    ...
    public QuestSpec parseOne(InputStream xmlStream, String filename) throws Exception {
        ...
        if (all.isEmpty()) {
            throw new IllegalArgumentException("No <quest> element found in quest file: " + filename);
        }
        if (all.size() > 1) {
            throw new IllegalArgumentException(
                    "Expected exactly 1 <quest> in file " + filename +
                    ", but found " + all.size() + ". Use parseAll() for multi-quest files.");
        }
        return all.get(0);
    }
